...
## Remote server Work (Alpine & Jravilab server)

For computing that requires high performance hardware, we utilize two different remote servers.

### Jravilab server

- A single high performance machine only for our group
- To get setup contact the system administrator `shaddai.amolitos@cuanschutz.edu` by sending him your CU username and a brief message that you will need access to the server. CC janani.ravi@cuanschutz.edu, so they can both verify your access.
- If you are new to remote server work, then it maybe helpful to learn about using an [ssh config](https://linuxize.com/post/using-the-ssh-config-file/) for manaing your remote hosts.

### Alpine

- Alpine is the HPC cluster for all the Colorado University campuses, including Anschutz.
